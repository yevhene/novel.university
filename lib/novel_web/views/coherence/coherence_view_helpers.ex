defmodule NovelWeb.Coherence.ViewHelpers do
  @moduledoc """
  ...
  @seperator {:safe, "&nbsp; | &nbsp;"}
  @helpers NovelWeb.Router.Helpers
  ...
  * :register - customize the register link text (#{@register_link})
  ...
  def coherence_links(conn, :layout, opts) do
    ...
    if Coherence.logged_in?(conn) do
      ...
      [
        nav_item_tag(list_tag, profile_link(current_user, conn)),
        nav_item_tag(list_tag, signout_link(conn, signout, signout_class))
      ]
    else
      signin_link = nav_item_tag(list_tag, nav_link(signin, to: coherence_path(@helpers, :session_path, conn, :new)))
      if Config.has_option(:registerable) && register do
        [nav_item_tag(list_tag, nav_link(register, to: coherence_path(@helpers, :registration_path, conn, :new))), signin_link]
      else
        signin_link
      end
    end
  ...
  def signout_link(conn, text \\ @signout_link, signout_class \\ "") do
    nav_link(text, to: coherence_path(@helpers, :session_path, conn, :delete), method: :delete, class: signout_class)
  end
  ...
  defp profile_link(current_user, conn) do
    if Config.user_schema.registerable? do
      nav_link current_user.name, to: coherence_path(@helpers, :registration_path, conn, :show)
    else
      current_user.name
    end
  end

  defp nav_item_tag(name, content) do
    content_tag(name, content, class: @nav_item_class)
  end

  defp nav_link(name, opts \\ []) do
    opts = Keyword.put opts, :class, @nav_link_class
